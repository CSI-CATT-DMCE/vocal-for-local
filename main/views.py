...
from VFL.settings import EMAIL_HOST_USER
...
def registerPage(request):
	if request.method=="POST":
		...
		if User.objects.filter(username=user).exists():
			messages.info(request,"A profile with that username already exists.")	
			...
			send_mail("Welcome to Link ", "your password is: "+pw,EMAIL_HOST_USER, [email])
			
			user=User.objects.create_user(username=user,password=pw,email=email)
